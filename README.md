...
non-essential effects and show essential boons/buffs in your own rows.
...
- **Hide the clutter.** Food, utility, boosts, guild and other non-essential
  ...
  the gap.
- **Rows for what matters.** Put important boons and buffs (Quickness,
  Alacrity, your profession's key buffs ...) in up to sixteen rows of your own:
  two at the top of the effect monitor, the rest anywhere on screen, each in
  your own order. They also keep their normal icon.
...
```
Row 1        [Quickness][Alacrity]          <- your rows
Row 2        [Might]
...
2. Press the key to hide or show the non-essential effects. The state is
   saved.
3. Click the eye icon in the Nexus quick-access bar for a quick window with
   your rows, active effects, categories and other-player settings
   (right-click it for just the two hide switches). Everything else is in Nexus > Addons > No-More-Effects.

## Options

**Hide non-essential effects** - the same switch as the key. Tick **Only in
combat** next to it to hide only while you are in combat

**Show quick-access icon** - shows or removes the eye icon in the Nexus
quick-access bar. Clicking the icon opens the quick window

**Rows** - **Add row** adds a row (up to 16), **Remove** deletes one. Each row is 
**In effect monitor** (the two spare rows above your boons, row 1 the topmost) or 
**Floating**: drawn by thegame anywhere on screen, with clicks and hover passing 
through it. **Edit layout** shows every floating row as a red box to drag into place

**Choose boon/buff rows** - give a boon or buff a row (**Row 1**,
**Row 2** ...) and it also shows there, even if it is hidden below.

For a boon or profession buff in a row, **Hide below** hides its normal
icon so it only shows in its row (while hiding is on). A boon is never hidden
otherwise:
Each profession keeps its own rows and order; swapping characters switches to
theirs. Tick **Account-wide** next to a row to give it the same boons/buffs
on every profession instead (handy for a boon row)
...
**Row** puts it in one of your rows. Hidden effects that expired are listed
...
**Only show selected boons on party members** keeps just the boons you tick 
on the party frames (normal and compact) and hides the rest there; the target 
frame always shows every boon (an enemy's Stability stays visible).
...
- The profession buff list covers buffs that belong to a single profession
  ...
  effect missing there can still be put in a row from Active Effects.
